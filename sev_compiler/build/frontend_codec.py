...
SKIP = {
    ...
    'model_syntax_block_block.Block': {'operations', 'lowered_operations', 'cfg_index', 'parameters', 'terminator', 'origins', 'origin'},
}
...
def exported_sources(entry):
    ...
    def visit(path):
        ...
        seen.add(path)
        for locator in re.findall(r'^import [\w ,]+ from "([^"\n]+)"$', path.read_text(), re.M):
...
def generate(root=ROOT, mir=False, interface=False, bodies=False):
    ...
    model_sources = [
        ...
        'sev_compiler/syntax/block/control_flow.sev',
...
import "package:semantic/semantic/src/definitions.sev" as definitions
...
'''
    ...
    def model_imports(directory):
        ...
        for namespace, paths in sorted(paths_by_namespace.items()):
            ...
            lines.append('import "' + os.path.relpath(path, directory) + '" as ' + namespace)
        return '\n'.join(lines) + '\n'
    ...
    imports = []
    outputs = {}
    codec_names = sorted(set(re.findall(r'^(?:class|def) (\w+)', header + '\n' + '\n'.join(optional_classes) + '\n' + '\n'.join(functions), re.M))) + ['schema']
    for index, chunk in enumerate(chunks):
        ...
        outputs[directory/name] = (model_imports(directory) + '# Generated by sev_compiler/build/frontend_codec.py.\n'
                                   'import syntax\nimport lexer\nimport source\nimport package\n'
                                   f'import "package:semantic/semantic/src/callable.sev" as semantic\n'
                                   f'import "package:semantic/semantic/src/definitions.sev" as definitions\n'
                                   'import ' + ', '.join(n for n in codec_names if n not in re.findall(r'^def (\w+)', '\n'.join(chunk), re.M)) + ' from "../frontend_archive.sev"\n\n\n' +
                                   '\n\n\n'.join(chunk) + '\n')
        imports.append('import ' + ', '.join(re.findall(r'^def (\w+)', '\n'.join(chunk), re.M)) + f' from "frontend_archive/{name}"')
    ...
    outputs[build/'frontend_archive.sev'] = (
        ...
        'import ' + ', '.join(codec_names) + f' from "{output.relative_to(build).as_posix()}"\n')
    if mir:
