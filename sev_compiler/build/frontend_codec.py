...
def generate(root=ROOT, mir=False):
    models = {}
    inputs = {}
    ...
    for contract in (('universal.Module',) if mir else ('universal.Module', 'lexer.SyntaxRegistry', 'list[source.SourceFile]', 'list[semantic.PreludeAnalysis]')): ensure(contract)
...
import * from "package:semantic/src/definitions.sev" as definitions
...
'''
    ...
    build = root / 'sev_compiler/boundaries/driver/package.pkg/build'
    ...
    for index, chunk in enumerate(chunks):
        ...
        outputs[directory/name] = ('# Generated by sev_compiler/build/frontend_codec.py.\n'
                                   'import universal\nimport lexer\nimport source\nimport package\n'
                                   f'import * from "package:semantic/src/callable.sev" as semantic\n'
                                   f'import * from "package:semantic/src/definitions.sev" as definitions\n'
