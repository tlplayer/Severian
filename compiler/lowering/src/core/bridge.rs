...
pub(super) fn native_bridge_source_for_target(
    ...
    source.push_str(concat!(
        ...
        "int64_t __sev_unbox_i64(void *raw) { sev_value *value = raw; if (!value || value->kind != SEV_INT) sev_runtime_fail(\"E000921\", \"value cannot be converted to int\", \"expected a runtime integer value\"); return value->as.i64; }\n",
        "double __sev_unbox_f64(void *raw) { sev_value *value = raw; if (!value || value->kind != SEV_FLOAT) sev_runtime_fail(\"E000921\", \"value cannot be converted to float\", \"expected a runtime float value\"); return value->as.f64; }\n",
        "bool __sev_unbox_bool(void *raw) { sev_value *value = raw; if (!value || value->kind != SEV_BOOL) sev_runtime_fail(\"E000921\", \"value cannot be converted to bool\", \"expected a runtime boolean value\"); return value->as.boolean; }\n",
        "void *__sev_unbox_string(void *raw) { sev_value *value = raw; if (!value || value->kind != SEV_STRING) sev_runtime_fail(\"E000921\", \"value cannot be converted to string\", \"expected a runtime string value\"); return (void *)value->as.string; }\n",
        "void *__sev_unbox_ptr(void *raw) { sev_value *value = raw; if (!value || value->kind != SEV_COLLECTION) sev_runtime_fail(\"E000921\", \"value is not a collection\", \"expected a runtime collection value\"); return value->as.pointer; }\n",
        ...
        "int64_t __sev_value_int(void *raw) { sev_value *value = raw; if (!value) sev_runtime_fail(\"E000921\", \"value cannot be converted to int\", \"the value is missing\"); if (value->kind == SEV_INT) return value->as.i64; if (value->kind == SEV_BOOL) return value->as.boolean ? 1 : 0; if (value->kind == SEV_FLOAT) { double result = value->as.f64; if (!isfinite(result) || result >= 9223372036854775808.0 || result < -9223372036854775808.0) sev_runtime_fail(\"E000921\", \"float cannot be converted to int\", \"the floating-point value is outside the int range\"); return (int64_t)result; } if (value->kind == SEV_STRING) { const char *text = value->as.string; char *end = NULL; errno = 0; long long result = strtoll(text, &end, 10); while (end && isspace((unsigned char)*end)) ++end; if (end == text || (end && *end != '\\0') || errno == ERANGE) sev_runtime_fail(\"E000921\", \"string cannot be converted to int\", text); return (int64_t)result; } sev_runtime_fail(\"E000921\", \"value cannot be converted to int\", \"expected an int, float, bool, or base-10 integer string\"); }\n",
        "double __sev_value_float(void *raw) { sev_value *value = raw; if (!value) sev_runtime_fail(\"E000921\", \"value cannot be converted to float\", \"the value is missing\"); if (value->kind == SEV_FLOAT) return value->as.f64; if (value->kind == SEV_INT) return (double)value->as.i64; if (value->kind == SEV_STRING) { char *end = NULL; double result = strtod(value->as.string, &end); if (end == value->as.string || *end != '\\0') sev_runtime_fail(\"E000921\", \"string cannot be converted to float\", value->as.string); return result; } sev_runtime_fail(\"E000921\", \"value cannot be converted to float\", \"expected an int, float, or numeric string\"); }\n",
    ));
    ...
    source.push_str(concat!(
        ...
        "bool __sev_map_contains(void *raw, void *key) { sev_map *value = raw; if (!value || !key) abort(); for (int64_t index = 0; index < value->size; ++index) if (sev_value_equal(value->keys[index], key)) return true; return false; }\n",
        "void *__sev_map_get(void *raw, void *key) { sev_map *value = raw; if (!value) sev_runtime_fail_invariant(\"map storage is null\"); for (int64_t i = 0; i < value->size; ++i) if (sev_value_equal(value->keys[i], key)) return value->values[i]; sev_runtime_fail(\"E000911\", \"map key was not found\", \"the requested key is not present in this map\"); }\n",
...
"#,
        ...
        "void *__sev_map_pop_required(void *raw, void *key) { sev_map *value = raw; if (!value) abort(); for (int64_t index = 0; index < value->size; ++index) if (sev_value_equal(value->keys[index], key)) { sev_value *result = value->values[index]; memmove(value->keys + index, value->keys + index + 1, (size_t)(value->size - index - 1) * sizeof(*value->keys)); memmove(value->values + index, value->values + index + 1, (size_t)(value->size - index - 1) * sizeof(*value->values)); value->size -= 1; return result; } sev_runtime_fail(\"E000911\", \"map key was not found\", \"the requested key is not present in this map\"); }\n",
