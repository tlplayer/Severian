...
use crate::{algebraic, fusion, layout, memory, scheduling};
use severian_hir::Program;
use std::collections::HashMap;

pub fn pipeline() -> severian_passes::PassManager {
    let mut pipeline = severian_passes::PassManager::default();
    pipeline.add(algebraic::AlgebraicSimplification::default());
    pipeline.add(fusion::InstructionFusion::default());
    pipeline
}
