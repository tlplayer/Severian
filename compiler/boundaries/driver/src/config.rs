...
impl Manifest {
    ...
    /// dependencies are frozen to registry package identities and development
    /// dependencies must not leak into a consumer's graph. Declared binaries
    /// remain available for `sev run PACKAGE` and `sev install PACKAGE`.
    ...
    pub fn render_lockfile(&self) -> Result<String, String> {
        let mut packages = self
            .package_graph
            .packages
            .values()
            .map(|package| {
                let version = manifest_package_version(&package.manifest);
                let mut dependencies = package
                    ...
                    .map(|id| {
                        let dependency = self.package_graph.packages.get(id).ok_or_else(|| {
                            format!(
                                "package `{}` has an unresolved lockfile dependency",
                                package.name
                            )
                        })?;
                        Ok(format!(
                            "{} {}",
                            dependency.name,
                            manifest_package_version(&dependency.manifest)
                        ))
                    })
                    .collect::<Result<Vec<_>, String>>()?;
                dependencies.sort();
        ...
        let mut output = String::from("# Generated by sev.\nversion = 1\n");
        for (name, version, source, checksum, dependencies) in packages {
            output.push_str("\n[[package]]\n");
...
fn package_source_kind(package: &ResolvedPackage) -> String {
    let registry_source = package
        ...
        && package
            ...
            .is_some_and(|version| version == manifest_package_version(&package.manifest).as_str())
        && package
            .root
            .parent()
...
fn package_checksum(package: &ResolvedPackage) -> Result<String, String> {
    let version = manifest_package_version(&package.manifest);
    let archive = package
        .root
        .parent()
        .map(|release| release.join(format!("{}-{version}.pkg", package.name)));
    let bytes = if let Some(archive) = archive.filter(|archive| archive.is_file()) {
        fs::read(&archive)
            .map_err(|error| format!("could not checksum {}: {error}", archive.display()))?
    } else {
        toml::to_string(&package.manifest)
            .map_err(|error| format!("could not checksum package `{}`: {error}", package.name))?
            .into_bytes()
    };
...
mod tests {
    ...
    fn published_source_manifest_freezes_path_dependencies_to_registry_identities() {
        ...
        assert!(value.get("bin").is_some());
        assert_eq!(value["package"]["default-run"].as_str(), Some("root"));
