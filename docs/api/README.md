...
The inventory covers the complete required topology and exact compiler-facing
surface:
...
Families may declare `members`; each member inherits the complete contract and
is independently addressable by stable ID. Library package records also bind
`export_sources` to their exact `export_symbols`, making source/API drift an
error.
...
The driver validates the schema, stable/unique IDs, statuses, snippets,
implemented-feature evidence, required topology, source export manifests,
Universal primitives/operators/literals/conversions, public AST expression,
statement and declaration variants, all structural tensor operation IDs, and
every tensor sub-operation variant.
The Severian checker additionally extracts every Markdown `sev` fence and runs
a sequential, CPU-pinned, memory-bounded compiler check. The symmetry runner
compares Severian behavior with small Python or Rust reference programs under
the same bounded execution policy; Python is an oracle only, never the
validator.
