...
| --- | --- | --- | --- | --- |
| `W-API-001` | API tooling | `sev api ...` does not exist; validation currently runs through self-hosted `docs/api/check.sev`. | No driver subcommand is registered. | Native `list`, `show`, `check`, and `diff` commands consume the same records. |
...
| `W-TEST-001` | Behavioral symmetry | Python/Rust symmetry currently covers scalar operators and ordinary generics, not the full documented API. | `docs/api/SYMMETRY.md` lists two passing cases and the missing groups. | Every implemented API group has a behavioral or compile-outcome oracle; partial groups expose failing cases at their precise boundary. |
