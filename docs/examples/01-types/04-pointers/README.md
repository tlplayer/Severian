...
the normal way to share values. A `pointer[T]` is used for foreign memory, allocated
...
- nullability is written `pointer[T] | None` rather than hidden inside every pointer;
