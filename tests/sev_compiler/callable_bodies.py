...
import os
from bootstrap_mlir import declared_native_requirements, ROOT, ARTIFACTS, SEED, run, tool
...
def main():
    ...
    for subject in sorted(subjects.glob("*.sev")):
        ...
        passes, libraries = declared_native_requirements(emitted.read_text())
        ...
        run([tool("SEVERIAN_CLANG", "clang-21"), llvm, "-o", output, "-lm", *libraries])
        actual = run([output])
        ...
        if os.environ.get("SEVERIAN_SANITIZE") == "1":
            ...
            run([tool("SEVERIAN_CLANG", "clang-21"), "-fsanitize=address", llvm, "-o", sanitized, "-lm", *libraries])
